Miguelkoin Core version 0.16.0 is now available from:

  <https://miguelkoincore.org/bin/miguelkoin-core-0.16.0/>
...
installer (on Windows) or just copy over `/Applications/Miguelkoin-Qt` (on Mac)
or `miguelkoind`/`miguelkoin-qt` (on Linux).
...
Miguelkoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
miguelkoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Miguelkoin Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µMKN" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- miguelkoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- miguelkoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 19443.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest miguelkoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.miguelkoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for miguelkoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to miguelkoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µMKN to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_miguelkoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` miguelkoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.miguelkoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for miguelkoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in miguelkoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop miguelkoind (OmeGak)
...
- #10529 `331352f` Improve miguelkoind systemd service file (Flowdalic)
