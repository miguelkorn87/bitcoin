#!/bin/bash
# Copyright (c) 2013 The Miguelkoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Miguelkoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Miguelkoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Miguelkoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Miguelkoin 0.7 datadir."
    ;;
  3)
    echo "Detected Miguelkoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Miguelkoin 0.8 datadir."
    ;;
esac
