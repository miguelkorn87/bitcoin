...
###   miguelkoincore.org and miguelkoin.org and compares them.
...
WORKINGDIR="/tmp/miguelkoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://miguelkoincore.org"
HOST2="https://miguelkoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="miguelkoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'miguelkoin-',
...
if [ $? -ne 0 ]; then
   echo "miguelkoin.org failed to provide signature file, but miguelkoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "miguelkoin.org and miguelkoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Miguelkoin Core binary release signing key installed?"
   fi
