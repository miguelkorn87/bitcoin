...
The RPM spec file provided here is for Miguelkoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://miguelkoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Miguelkoin git repository](https://github.com/miguelkoin/miguelkoin) but are not part of
...
Currently only `Source10` is used, for the example `miguelkoin.conf` file.
...
in the miguelkoin git repository. At present only an SVG version of the Miguelkoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the miguelkoin
...
needed when 0.12.1 is released, a proper fix is already in the Miguelkoin git
master branch.
...
idea to build Miguelkoin Core against a static version of that library compiled
...
The `miguelkoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `miguelkoin-server` package. At this point, most RPM based
...
The `miguelkoin-server` package should have SELinux support. How to properly do
...
Miguelkoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/miguelkoin/). He has been
packaging Miguelkoin for Fedora at least since 2012.
...
4. I renamed the `miguelkoin` package that contains the Qt GUI to `miguelkoin-core` as
that appears to be how the general population refers to it, in contrast to
`miguelkoin-xt` or `miguelkoin-classic`. I wanted to make sure the general population
