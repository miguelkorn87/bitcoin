    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/miguelkoin/miguelkoin/issues/5716
...
{
    GUIUtil::setClipboard(MiguelkoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = MiguelkoinUnits::MKN;
    ...
    l2->setText(MiguelkoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(MiguelkoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(MiguelkoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(MiguelkoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    for (const std::pair<QString, std::vector<COutput>>& coins : mapCoins) {
        ...
        for (const COutput& out : coins.second) {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show miguelkoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, MiguelkoinUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, MiguelkoinUnits::format(nDisplayUnit, nSum));
